...
// each group's Dijkstra run can't be influenced by the others. Exercising dijkstra/
// reconstructPath/convertPathToRoute directly (rather than mocking networkData/RMP.json)
// gives exact control over tie-breaking, leg grouping, and exclusion detours, which the
// real dataset doesn't conveniently exercise, without pulling in the RMP-map machinery
// that findRoute's module-level singleton would otherwise require mocking.
const lines = [
...
function findMultiStopRoute(
    ...
    for (let i = 0; i < stationNames.length - 1; i++) {
        const leg = findRoute(stationNames[i], stationNames[i + 1], metric, exclusions);

        if (leg.length === 0) return { ok: false, failedHop: [stationNames[i], stationNames[i + 1]] };
        hops.push(leg);
    }
